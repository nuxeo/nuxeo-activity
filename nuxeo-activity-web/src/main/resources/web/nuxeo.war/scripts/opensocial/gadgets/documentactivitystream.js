...
(function() {
  ...
  var documentPath = gadgets.util.unescapeString(prefs.getString("nuxeoTargetContextPath"));
  ...
  function buildActivityHtml(template, activity) {
    activity = computeActivityIcon(activity);
    return Mustache.render(template, activity);
  }

  function computeActivityIcon(activity) {
  ...
  function showMoreActivities() {
    var NXRequestParams= { operationId : 'Services.GetDocumentActivityStream',
      operationParams: {
        language: prefs.getLang(),
        contextPath: documentPath,
